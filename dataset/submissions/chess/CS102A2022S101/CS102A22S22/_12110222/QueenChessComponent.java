        ...
        for (int i = x; i <8 ; i++) {
            if(x==7){break;}
                if(source.offset(i-x+1,0)!=null){
                    if(components[i+1][y].getChessColor()==chessColor){break;}
                    list.add(new ChessboardPoint(i+1,y));
                    if(components[i+1][y].getChessColor()!=chessColor&&components[i+1][y].getChessColor()!=ChessColor.NONE){break;}
            }
        }
...
if(y==0){break;}
            if(source.offset(0,i-y-1)!=null){
                if(components[x][i-1].getChessColor()==chessColor){break;}
                list.add(new ChessboardPoint(x,i-1));
                if(components[x][i-1].getChessColor()!=chessColor&&components[x][i-1].getChessColor()!=ChessColor.NONE){break;}
            }
        }
        for (int i = y; i <8 ; i++) {
if(y==7){break;}
