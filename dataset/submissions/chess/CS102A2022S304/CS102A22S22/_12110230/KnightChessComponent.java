...
public class KnightChessComponent extends ChessComponent
{
    ...
    public List<ChessboardPoint> canMoveTo()
    {
        ...
        shouldMove.add(new ChessboardPoint(getSource().getX()+2, getSource().getY()-1));
        shouldMove.add(new ChessboardPoint(getSource().getX()+1, getSource().getY()-2));
        shouldMove.add(new ChessboardPoint(getSource().getX()-1, getSource().getY()-2));
        shouldMove.add(new ChessboardPoint(getSource().getX()-2, getSource().getY()-1));
        shouldMove.add(new ChessboardPoint(getSource().getX()-2, getSource().getY()+1));
        shouldMove.add(new ChessboardPoint(getSource().getX()-1, getSource().getY()+2));
        ...
        for (ChessboardPoint i:shouldMove)
        {
            ...
            {
                if(Chessboard[i.getX()][i.getY()] instanceof EmptySlotComponent || Chessboard[i.getX()][i.getY()].getChessColor()!=color) {output.add(i);}
            }
