...
public class KnightChessComponent extends ChessComponent {
    ...
    public boolean canMoveTo(ChessboardPoint destination) {
        ChessboardPoint source = getSource();
        return (Math.abs(destination.getX() - source.getX()) + Math.abs(destination.getY() - source.getY())) == 3 && Math.abs(destination.getX() - source.getX()) > 0 && Math.abs(destination.getY() - source.getY()) > 0;
    }
}