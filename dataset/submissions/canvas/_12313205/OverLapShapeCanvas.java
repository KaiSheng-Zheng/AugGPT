...
public class OverLapShapeCanvas implements ShapeCanvas{
    ...
    public boolean addShape(int x, int y, char pattern, int... params) {
        ...
        if (params.length == 1) {
            ...
            if (newCircle.area() - outCanvasBoundaryCount > 0) {

                shapes.add(newCircle);

                for (int i = 0; i < newCircle.grids.length; i++) {
                    for (int j = 0; j < newCircle.grids[0].length; j++) {
                        if (newCircle.grids[i][j] == pattern && x+i <canvas.length && y+j < canvas[0].length) {
        ...
        if (params.length == 3) {
            ...
            if (newRightTriangle.area() - outCanvasBoundaryCount > 0) {
                shapes.add(newRightTriangle);

                for (int i = 0; i < newRightTriangle.grids.length; i++) {
                    for (int j = 0; j < newRightTriangle.grids[0].length; j++) {
                        if (newRightTriangle.grids[i][j] == pattern && x+i <canvas.length && y+j < canvas[0].length) {
                            canvas[x + i][y + j] = pattern;
                        }
                    }
                }
