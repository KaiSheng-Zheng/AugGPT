...
/**
 * The OverLapShapeCanvas class implements the ShapeCanvas interface and allows shapes to overlap on the canvas.
...
public class OverLapShapeCanvas implements ShapeCanvas
{
    ...
    /**
     * Attempts to add a new shape to the canvas. If the shape can be fully placed within the canvas, the addition is successful.
     *
    ...
    public boolean addShape(int x, int y, char pattern, int... params)
    {
        ...
        // Check if it can be placed on the canvas
    ...
    /**
     * Checks whether the shape can be placed at the specified position.
     * This method traverses each cell of the shape to check whether it will exceed the boundary of the canvas after placement.
     * Here, as long as a part of this shape exists on the canvas, it can be inserted, and it can go beyond the boundary (only the part that exists on the canvas is finally saved).
     *
     * @param shape The shape to be placed
     * @param x     The starting x-coordinate of the shape on the canvas
     * @param y     The starting y-coordinate of the shape on the canvas
     * @return true if the shape can be placed at the specified position, otherwise false
     */
    private boolean canPlaceShape(Shape shape, int x, int y)
    {
        // Get the shape to be inserted
        char[][] shapeGrids = shape.getGrids();
        // Check if the shape exists on the canvas
        for (int i = 0; i < Math.min(canvas.length - x, shapeGrids.length); i++)
        {
            for (int j = 0; j < Math.min(canvas[0].length - y, shapeGrids[i].length); j++)
            {
                if (shapeGrids[i][j] != ' ')
                {
                    return true;  // There is a shape on the canvas, the shape can be placed
                }
            }
        }
        return false;
     ...
     * This method traverses each cell of the shape and places the cell of the shape in the corresponding position of the canvas (note that the part that exceeds the canvas should not be inserted, it will exceed the boundary).
     *
     * @param shape The shape to be placed
     * @param x     The starting x-coordinate of the shape on the canvas
     * @param y     The starting y-coordinate of the shape on the canvas
