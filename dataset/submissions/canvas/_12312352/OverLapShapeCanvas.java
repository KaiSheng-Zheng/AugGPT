    ...
    public boolean addShape(int x, int y, char pattern, int... params) {
        ...
        for (int i = 0; i < aShape.getGrids().length; i++){
            for (int j = 0; j < aShape.getGrids()[i].length; j++) {
                boolean s=aShape.getGrids()[i][j]!=' ';
                boolean d=x+i< canvas.length&&x+i>=0;
                boolean f=(y+j)< canvas[0].length&&(y+j)>=0;
                if (s&&d&&f){
                    //if (s&&d&&f){
                    canvas[x+i][y+j]=aShape.getGrids()[i][j];
                    a=true;
                }
                /*if ((aShape.getGrids()[i][j]!=' '&&(x+i)< canvas.length&&(x+i)>=0)&&(aShape.getGrids()[i][j]!=' '&&((y+j)< canvas[0].length&&(y+j)>=0))){
