...
public class AvoidConflictShapeCanvas implements ShapeCanvas {
    ...
    public boolean addShape(int x, int y, char pattern, int... params) {
        if (params.length == 1) {
            ...
            if (y + 2 * params[0] -1> column -1  || x + 2 * params[0]-1 > row-1 ) {
                return false;
            } else {
                circle.fillGrids();
                for(int Row= 0 ;Row< params[0]*2;Row++)
                {
                    ...
                    {
                        if(circle.grids[Row][Column]==pattern&&canvas[x+Row][y+Column]!=' ')
                        {
                            return false;
                        }
                    }
                ...
                for(int Row= 0 ;Row< params[0]*2;Row++)
                {
                    for(int Column = 0 ; Column < params[0]*2;Column++)
                    {
                        if(circle.grids[Row][Column]==pattern)
            ...
            if(x+params[1]-1>row-1||y+params[0]-1>column-1)
            {
                return false;
            }
            else{
                ...
                for(int Row= 0 ;Row< params[1];Row++)
                {
                    ...
                    {
                        if(rightTriangle.grids[Row][Column]==pattern&&canvas[x+Row][y+Column]!=' ')
                        {
                            return false;
                        }
                    }
                ...
                for(int Row= 0 ;Row< params[1];Row++)
                {
                    for(int Column = 0 ; Column < params[0];Column++)
                    {
                        if(rightTriangle.grids[Row][Column]==pattern)
                        {
